        ...
        if self.platform == "ASCEND":
            ...
            if format == 'json':
                ...
                for _i in range(len(pred)):
                    pose_kpts = []
                    for _k, _kpt in enumerate(pred_kpts[_i]):
                        x, y, confidence = _kpt
                        pose_kpts.append({
                            "keypoint_id": _k,
                    ...
                    pose_detections.append({
                        ...
                        "keypoints": pose_kpts,  # List of keypoints
                    })
                    
                # Convert to JSON
                return json.dumps(pose_detections, indent=4)
                
            #results = Results(img0, path=img0_path, names=self.names, boxes=pred[:, :6], keypoints=pred_kpts)
...
if __name__ == "__main__":
    ...
    config_path='config/yolov8n-pose.json'
