...
class AclLiteModel(object):
    ...
    def _gen_input_dataset(self):
        ...
        for i in range(self._input_num):
            ...
            self._input_dataset, ret = acl.mdl.add_dataset_buffer(self._input_dataset, input_data)
            if ret != const.ACL_SUCCESS:
                print('acl.mdl.add_dataset_buffer failed, errorCode is', ret)
            self.input_buffers.append(input_buffer)
            self.input_buffer_sizes.append(input_buffer_size)
    ...
    def _set_dynamic_batch_size(self, batch):
        ...
        if ret != const.ACL_SUCCESS:
            ...
            return const.FAILED
        log_info("get dynamic_batch = ", batch_dic)
        ...
        if batch in batch_dic["batch"]:
            ...
            log_info("dynamic batch {} is not in {}".format(batch, batch_dic["batch"]))
            return const.FAILED
    ...
    def execute(self, input_list):
        ...
        if self._run_mode == const.ACL_DEVICE:
            kind = const.ACL_MEMCPY_DEVICE_TO_DEVICE
        else:
            kind = const.ACL_MEMCPY_HOST_TO_DEVICE
        for i,data_in in enumerate(input_list):
            if isinstance(data_in, AclLiteImage):
                size = data_in.size
                data = data_in.data()
            elif isinstance(data_in, np.ndarray):
                size = data_in.size * data_in.itemsize
                if "bytes_to_ptr" in dir(acl.util):
                    bytes_data = data_in.tobytes()
                    ptr = acl.util.bytes_to_ptr(bytes_data)
                else:
                    ptr = acl.util.numpy_to_ptr(data_in)
                data = ptr
                ...
                log_error("Unsupport input!")
            if size != self.input_buffer_sizes[i]:
                log_warning(" Input[%d] size: %d not equal om size: %d" % (i, size, self.input_buffer_sizes[i]) +\
                        ", may cause inference result error, please check model input")

            ret = acl.rt.memcpy(self.input_buffers[i],
                                self.input_buffer_sizes[i],
                                data,
                                self.input_buffer_sizes[i],
                                kind)
        ...
        numpy_data = self._output_dataset_to_numpy()
        return numpy_data
    ...
    def destroy(self):
        ...
        if self._model_id:
            ...
            if ret != const.ACL_SUCCESS:
                log_error("acl.mdl.unload error:", ret)

        if self._model_desc:
            ret = acl.mdl.destroy_desc(self._model_desc)
            if ret != const.ACL_SUCCESS:
                log_error("acl.mdl.destroy_desc error:", ret)
